...
def split_val(dataset: pd.DataFrame, **kwargs):
    """
    Разделение данных на train/val с последующим сохранением
    :param dataset: датасет
    :return: train/test датасеты
    """
    # Split in train/test
    df_train_, df_val = train_test_split(
        dataset,
        test_size=kwargs["test_size_val"],
        random_state=kwargs["random_state"],
    )
    df_train_.to_csv(kwargs["train_val_path_proc"], index=False)
